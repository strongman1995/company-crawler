...
public class HtmlNode extends AbstractSelectable {
    ...
    public Selectable $(String selector, String attrName) {
        ...
        if(attrName.equals("allText")){
            res = $(selector);
            if(res ==null || StringUtils.isEmpty(res.get())){
                res = new PlainText(StringUtils.EMPTY);
            }else {
                Document doc = Jsoup.parse(res.get());
                res = new PlainText(doc.text());
            }
        }else {
            CssSelector cssSelector = Selectors.$(selector, attrName);
            res = selectElements(cssSelector);
        }
        return res;
    }
